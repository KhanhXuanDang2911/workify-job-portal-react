...
export default function CategoryJobs() {
  ...
  const [selectedCategoryJob, setSelectedCategoryJob] =
    useState<CategoryJobResponse | null>(null);
  ...
  useEffect(() => {
    if (
      categoryJobsData &&
      categoryJobsData.data.length > 0 &&
      !selectedCategoryJob
    ) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="w-1/2 border-r border-gray-200 bg-white overflow-y-auto">
        <CategoryJobTable
          onSelectJob={setSelectedCategoryJob}
          selectedCategoryJob={selectedCategoryJob}
        />
      </div>
      ...
      {selectedCategoryJob ? (
        ...
        <div className="w-1/2 flex items-center justify-center bg-gray-50">
          <p className="text-gray-400 text-lg">
            {t("admin.categoryJobManagement.selectCategory")}
          </p>
        </div>
      )}
    </div>
