            ...
            <tr className="bg-muted/40 text-left text-gray-700 dark:text-gray-300 border-b">
              ...
              <th className="px-4 py-3 font-semibold uppercase tracking-wide text-xs">
                <div className="flex items-center justify-start gap-2">
                  ...
                  <MultiSortButton direction={sorts.find((s) => s.field === "createdAt")?.direction ?? null} onChange={(newDirection) => handleSortChange("createdAt", newDirection)} />
                </div>
              </th>
              <th className="px-4 py-3 font-semibold uppercase tracking-wide text-xs">
                <div className="flex items-center justify-start gap-2">
                  <span>Updated At</span>
                  <MultiSortButton direction={sorts.find((s) => s.field === "updatedAt")?.direction ?? null} onChange={(newDirection) => handleSortChange("updatedAt", newDirection)} />
                </div>
              </th>
            ...
            {isLoadingEmployers ? (
              ...
              employers.map((employer) => (
                <tr key={employer.id} className="hover:bg-muted/30 border-b last:border-none transition-colors">
                  ...
                  <td className="px-4 py-3 font-medium text-gray-900 dark:text-gray-100 max-w-[200px] truncate">{employer.companyName}</td>
                  <td className="px-4 py-3">
                    <Badge variant="outline" className={UserStatusColors[employer.status]}>
                      {UserStatusLabelEN[employer.status]}
                    </Badge>
                  </td>
                  <td className="px-4 py-3">{CompanySizeLabelEN[employer.companySize]}</td>
                  <td className="px-4 py-3">{employer.email}</td>
                  <td className="px-4 py-3">{employer.district?.name}</td>
                  <td className="px-4 py-3">{employer.province?.name}</td>
                  <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{new Date(employer.createdAt).toLocaleDateString("vi-VN")}</td>
                  <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{new Date(employer.updatedAt).toLocaleDateString("vi-VN")}</td>
