...
export default function ProvincesTable({ onSelectProvince, selectedProvince }: ProvincesTableProps) {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(filteredProvinces?.map((item) => item.id) || []);
    } else {
      setSelectedIds([]);
    }
  };
  ...
  const handleSearch = () => {
    ...
    const filtered = provincesData?.filter(
      (province) => province.name.toLowerCase().includes(keyword) || province.engName.toLowerCase().includes(keyword) || province.code.toLowerCase().includes(keyword)
    );
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1 flex gap-2">
          <Input
            type="text"
            placeholder="Search by name,code, eng name..."
            value={searchInput}
          ...
          <thead>
            <tr className="bg-gray-100 border-b text-sm border-gray-200">
              <th className="px-4 py-3 text-left">
                <Checkbox checked={selectedIds.length === filteredProvinces.length && filteredProvinces.length > 0} onCheckedChange={handleSelectAll} />
              </th>
              <th className="px-4 py-3 text-left">
            ...
            {isLoadingProvincesData ? (
              ...
              <tr>
                <td colSpan={6} className="text-center py-10 text-muted-foreground">
                  ...
                  <p className="mt-2 text-sm text-gray-500">No provinces found</p>
                </td>
              </tr>
            ) : (
              filteredProvinces.map((province) => (
                <tr
                  ...
                  className={cn("border-b border-gray-200 text-[13px] hover:bg-gray-50 cursor-pointer", province.id === selectedProvince?.id && "bg-green-200")}
                >
                  <td className="px-4 py-3">
                    <Checkbox
                      onClick={(e) => e.stopPropagation()}
                      checked={selectedIds.includes(province.id)}
                      onCheckedChange={(checked) => handleSelectOne(province.id, checked as boolean)}
                    />
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <span className="px-3 py-1 rounded-full bg-gray-100">{province.engName}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="px-3 py-1 rounded-full bg-gray-100">{province.code}</span>
                  </td>
                  <td className="px-4 py-3 text-gray-600">{new Date(province?.createdAt as string).toLocaleDateString("vi-VN")}</td>
                  <td className="px-4 py-3 text-gray-600">{new Date(province?.updatedAt as string).toLocaleDateString("vi-VN")}</td>
                </tr>
              ))
            )}
          </tbody>
