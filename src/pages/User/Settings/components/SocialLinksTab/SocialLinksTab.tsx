      ...
      <div className="space-y-4">
        {socialLinks.map((link, index) => (
          <div key={link.id} className="space-y-2">
            <Label className="text-sm font-medium text-gray-700">Social Link {index + 1}</Label>
            <div className="flex gap-3 items-start">
              {/* Platform Select */}
              <Select value={link.platform} onValueChange={(val) => updateSocialLink(link.id, "platform", val)}>
                <SelectTrigger className="w-[200px] focus-visible:ring-1 focus-visible:ring-[#1967d2]">
                  <SelectValue>
                    <div className="flex items-center gap-2">
                      <span>{socialPlatforms.find((p) => p.value === link.platform)?.icon}</span>
                ...
                <SelectContent>
                  {socialPlatforms.map((platform) => (
                    <SelectItem key={platform.value} value={platform.value} className="focus:bg-sky-200 focus:text-[#1967d2]">
                      <div className="flex items-center gap-2">
              ...
              {/* Remove Button */}
              <Button type="button" variant="ghost" size="icon" onClick={() => removeSocialLink(link.id)} className="hover:bg-red-50 hover:text-red-600">
                <X className="w-5 h-5" />
              </Button>
            </div>
          </div>
