...
import { createPasswordSchema, type CreatePasswordFormData } from "@/schemas/auth.schema";
...
export default function CreatePassword() {
  ...
  const createPasswordMutation = useMutation({
    mutationFn: (password: string) => authService.createPassword(token!, password),
    onSuccess: (response) => {
    ...
    <div className="main-layout">
      <div className="flex min-h-screen">
        ...
        <div className="hidden lg:flex lg:w-1/2 items-center justify-center ">
          <img src="/create-password-Illustration.png" alt="" className="w-full" />
        </div>
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center px-8 -mt-6">
          <div className="w-full max-w-md space-y-6">
            <div className="text-center space-y-2">
              <h2 className="text-3xl font-bold text-[#1967d2]">Create Password</h2>
            ...
            <Card className="border border-gray-300">
              <CardContent className="p-6">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <div className="mt-3 space-y-2">
                      {pwdRequirements.map((requirement, index) => (
                        <div key={index} className={cn("text-sm flex items-center gap-2", checkRequirement(requirement) ? "text-green-600" : "text-gray-500")}>
                          {checkRequirement(requirement) ? <Check className="w-4 h-4" /> : <span className="w-1.5 h-1.5 rounded-full bg-current ml-1" />}
                  ...
                  <Button
                    type="submit"
                    disabled={createPasswordMutation.isPending || !allRequirementsMet || !password || !confirmPassword}
