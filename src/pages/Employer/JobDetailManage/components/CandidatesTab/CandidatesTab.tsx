...
export default function CandidatesTab() {
  ...
  const [receivedWithin, setReceivedWithin] = useState<number | undefined>(undefined);
  ...
  const {
    ...
    queryKey: ["applications-by-job", Number(jobId), currentPage, pageSize, statusFilter, receivedWithin],
    queryFn: () =>
  ...
  if (isError) {
    return (
      <div className="py-6 bg-sky-50 flex items-center justify-center min-h-[400px]">
        <p className="text-gray-600">Có lỗi xảy ra khi tải danh sách ứng viên</p>
      </div>
    );
  }
    ...
    <div className="py-6 bg-sky-50">
      <div className="flex items-center justify-between mx-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <Badge className="bg-teal-600 hover:bg-teal-700">{totalCandidates}</Badge>
        </div>
        <div className="flex items-center gap-2">
          <Button
