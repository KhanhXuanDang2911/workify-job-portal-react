...
import { BriefcaseBusiness, FolderKanban, GraduationCap, GripVertical, Hand, Hash, Info, Medal, Pencil, ShieldCheck, SquareUser, Target, ThumbsUp, UserStar } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-4">Kéo và thả mục bất kỳ vào vị trí bạn muốn thêm trên CV</p>
          ...
          {unusedBlocks.map((block) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-sm group-hover:text-green-500">{block.label}</span>
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-4">Click để xem vị trí của mục trên CV</p>
        <div className="space-y-2">
          {usedBlocks.map((block) => {
            const Icon = block.icon;
            return (
              <Button key={block.id} data-id={block.id} variant="outline" className="group w-full justify-start text-gray-500 gap-3 h-auto py-3 bg-gray-100">
