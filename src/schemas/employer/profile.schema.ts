...
export const employerProfileUpdateSchema = z.object({
  ...
  contactPerson: z.string().min(1, "Tên người liên hệ là bắt buộc").min(2, "Tên người liên hệ phải có ít nhất 2 ký tự").max(100, "Tên người liên hệ không được vượt quá 100 ký tự"),
  phoneNumber: z.string().min(1, "Số điện thoại là bắt buộc").regex(PHONE_REGEX, "Số điện thoại không hợp lệ"),
...
export type EmployerProfileUpdateFormData = z.infer<typeof employerProfileUpdateSchema>;
...
export type EmployerWebsiteUpdateFormData = z.infer<typeof employerWebsiteUpdateSchema>;
